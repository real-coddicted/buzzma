...
public class DeliveryScreenshotScorer implements ClaimScreenshotScorer {
  ...
  public DeliveryScreenshotScorer(
  ...
  public void score(final ScoringJob job, final ClaimScreenshot screenshot) {
    LOGGER.info(
        "scoreDeliveryScreenshot: scoring job {}, screenshot {}", job.getId(), screenshot.getId());
    ...
    final ExtractedScoredResult extractedScoredResult =
        ClaimScreenshotScorerUtils.updateExtractedDataForMatchWithManualEntryInDelivery(
    ...
    LOGGER.info("scoreDeliveryScreenshot: saved score for screenshot {}", screenshot.getId());
  }
}
