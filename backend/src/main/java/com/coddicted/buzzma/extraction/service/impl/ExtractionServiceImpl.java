...
public class ExtractionServiceImpl extends BaseCrudService implements ExtractionService {
  ...
  @Override
  @Transactional
  public ExtractionJob extractSync(
    ...
    final List<ValidationError> errors = validator.validate(result);
    if (!errors.isEmpty()) {
  ...
  public ExtractionJob submitJob(
    ...
    final String storageKey =
        storageService.store(STORAGE_FOLDER, originalFilename, contentType, imageBytes);
    ...
    final ExtractionJob job =
        ExtractionJob.builder()
            ...
            .updatedBy(requesterId)
            .build();
    final ExtractionJob saved = jobRepository.save(job);
  ...
  public ExtractionJob processJob(final UUID jobId) {
    ...
    try {
      final byte[] imageBytes = storageService.retrieve(job.getStorageKey());
  ...
  public List<ExtractionJob> listByUser(final UUID requesterId) {
    return jobRepository.findBySubmittedByAndIsDeletedFalse(requesterId);
  }

  private ExtractionResult callGemini(final byte[] imageBytes, final String mimeType) {
    final String rawText =
        geminiClient.generateContent(promptBuilder.build(), imageBytes, mimeType);
