...
public class CampaignServiceImpl extends BaseCrudService implements CampaignService {
    ...
    public Campaign delete(final UUID campaignId, final UUID requesterId) {
        ...
        final Campaign updatedCampaign = existingCampaign.toBuilder().isDeleted(true).updatedBy(requesterId).build();
        return campaignRepository.save(updatedCampaign);
    }

    @Override
    @Transactional
    public Campaign action(final UUID campaignId, final CampaignAction campaignAction, final UUID requesterId) {
        final CampaignStatus target = switch (campaignAction) {
            case CAMPAIGN_ACTION_PUBLISH, CAMPAIGN_ACTION_RESUME -> CampaignStatus.CAMPAIGN_STATUS_ACTIVE;
    ...
    public Campaign copy(final UUID campaignId, final UUID requesterId) {
        ...
        final List<UUID> srcAssignmentIds = campaignAssignmentRepository.findByCampaignId(campaignId)
            .stream().map(CampaignAssignment::getId).toList();
    ...
    private Campaign transitionTo(final UUID campaignId, final CampaignStatus target, final UUID requesterId) {
        ...
        if (isPublish) {
            ...
            if (assignments.isEmpty()) {
                throw new BusinessRuleViolationException("Campaign must have at least one assignment before publishing");
            }
