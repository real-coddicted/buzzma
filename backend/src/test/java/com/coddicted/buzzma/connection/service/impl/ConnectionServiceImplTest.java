...
class ConnectionServiceImplTest {
  ...
  @Test
  void testCreateConnectionFromInviteCodeWhenAgencyInvitesBrandThenBrandBecomesParent() {
    doReturn(INVITE_ACTIVE).when(this.mockInviteService).getByCode(INVITE_CODE);
  ...
  @Test
  void testCreateConnectionFromInviteCodeWhenBrandInvitesAgencyThenBrandStaysParent() {
    doReturn(INVITE_ACTIVE).when(this.mockInviteService).getByCode(INVITE_CODE);
    doReturn(BRAND_USER).when(this.mockUserService).getById(FROM_USER_ID);

    this.connectionService.createConnection(INVITE_CODE, AGENCY_REQUESTER_USER);
