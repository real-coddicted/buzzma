...
import com.coddicted.buzzma.shared.security.UserAccessGuard;
...
@Import({TestSecurityConfig.class, UserAccessGuard.class})
class UsersControllerTest {
  ...
  void testGetByIdReturnsUserSummary() throws Exception {
  ...
  @Test
  @WithBuzzmaUser(role = UserRole.ROLE_BUYER)
  void testGetBankingWhenConnectedReturnsOk() throws Exception {
    when(connectionService.isConnected(any(), any())).thenReturn(true);
    final UserBankingDetail detail = UserBankingDetail.builder().userId(TARGET_USER_ID).build();
    when(userBankingDetailService.getByUserId(TARGET_USER_ID)).thenReturn(detail);
    when(userBankingDetailMapper.toDto(detail)).thenReturn(UserBankingDetailDto.builder().build());
  ...
  @Test
  @WithBuzzmaUser(role = UserRole.ROLE_BUYER)
  void testGetBankingWhenNotConnectedReturnsForbidden() throws Exception {
    when(connectionService.isConnected(any(), any())).thenReturn(false);
  ...
  @Test
  @WithBuzzmaUser(role = UserRole.ROLE_BUYER)
  void testGetBankingWhenConnectedButNoBankingDetailReturnsNotFound() throws Exception {
    when(connectionService.isConnected(any(), any())).thenReturn(true);
