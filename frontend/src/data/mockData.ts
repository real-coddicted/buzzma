import type {
  StatCardData,
  ActivityItem,
  Campaign,
  PerformanceBar,
  SecurityQuestion,
  UserDetails,
  Connection,
  Deal,
  PlatformOption,
  DealTypeOption,
  LinkedEntity,
} from '../types'

type RawDeal = Omit<Deal, 'platformLabel' | 'dealTypeLabel'>

export const statCards: StatCardData[] = [
  {
    id: 'total-campaigns',
    label: 'Total Campaigns',
    value: '48',
    subValue: '12 active',
    trend: 'up',
    trendValue: '+8 this month',
    accent: 'cyan',
    icon: 'megaphone',
  },
  {
    id: 'active-users',
    label: 'Active Users',
    value: '124,391',
    subValue: 'across all campaigns',
    trend: 'up',
    trendValue: '+12.4%',
    accent: 'green',
    icon: 'users',
  },
  {
    id: 'total-revenue',
    label: 'Total Revenue',
    value: '$284,520',
    subValue: 'MTD',
    trend: 'up',
    trendValue: '+23.1%',
    accent: 'purple',
    icon: 'currency',
  },
  {
    id: 'conversion-rate',
    label: 'Conversion Rate',
    value: '4.72%',
    subValue: 'avg. across channels',
    trend: 'down',
    trendValue: '-0.3%',
    accent: 'orange',
    icon: 'target',
  },
]

export const recentActivity: ActivityItem[] = [
  {
    id: 'act-1',
    type: 'campaign_launched',
    message: 'Campaign launched',
    detail: '"Summer Sale 2025" went live on Email & Social',
    timestamp: '2 min ago',
    accent: 'green',
  },
  {
    id: 'act-2',
    type: 'goal_reached',
    message: 'Conversion goal reached',
    detail: '"Q2 Retargeting" hit 10k conversions target',
    timestamp: '18 min ago',
    accent: 'cyan',
  },
  {
    id: 'act-3',
    type: 'budget_alert',
    message: 'Budget threshold hit',
    detail: '"Brand Awareness Q2" at 85% of budget',
    timestamp: '1 hr ago',
    accent: 'orange',
  },
  {
    id: 'act-4',
    type: 'campaign_paused',
    message: 'Campaign paused',
    detail: '"Holiday Clearance" paused pending creative review',
    timestamp: '3 hrs ago',
    accent: 'yellow',
  },
  {
    id: 'act-5',
    type: 'report_ready',
    message: 'Weekly report ready',
    detail: 'Performance digest for Apr 14–20 is available',
    timestamp: '5 hrs ago',
    accent: 'blue',
  },
  {
    id: 'act-6',
    type: 'new_user',
    message: 'New team member',
    detail: 'Sofia Chen joined as Campaign Analyst',
    timestamp: 'Yesterday',
    accent: 'pink',
  },
]

export const campaigns: Campaign[] = [
  {
    id: 'cmp-1',
    name: 'Summer Sale 2025',
    status: 'active',
    channel: 'email',
    budget: 15000,
    spent: 4320,
    impressions: 284000,
    clicks: 18900,
    conversions: 1240,
    ctr: 6.65,
    startDate: '2025-04-20',
    endDate: '2025-05-31',
  },
  {
    id: 'cmp-2',
    name: 'Q2 Retargeting Push',
    status: 'active',
    channel: 'social',
    budget: 22000,
    spent: 14800,
    impressions: 1120000,
    clicks: 47300,
    conversions: 3890,
    ctr: 4.22,
    startDate: '2025-04-01',
    endDate: '2025-06-30',
  },
  {
    id: 'cmp-3',
    name: 'Brand Awareness Q2',
    status: 'active',
    channel: 'display',
    budget: 18000,
    spent: 15300,
    impressions: 3400000,
    clicks: 22100,
    conversions: 580,
    ctr: 0.65,
    startDate: '2025-04-01',
    endDate: '2025-06-30',
  },
  {
    id: 'cmp-4',
    name: 'Product Launch — Nova',
    status: 'active',
    channel: 'video',
    budget: 40000,
    spent: 9600,
    impressions: 720000,
    clicks: 31400,
    conversions: 2150,
    ctr: 4.36,
    startDate: '2025-04-15',
    endDate: '2025-05-15',
  },
  {
    id: 'cmp-5',
    name: 'Spring Lead Gen',
    status: 'active',
    channel: 'search',
    budget: 12000,
    spent: 8900,
    impressions: 198000,
    clicks: 21600,
    conversions: 1780,
    ctr: 10.91,
    startDate: '2025-03-15',
    endDate: '2025-05-01',
  },
  {
    id: 'cmp-6',
    name: 'Holiday Clearance',
    status: 'paused',
    channel: 'email',
    budget: 8000,
    spent: 3100,
    impressions: 94000,
    clicks: 5800,
    conversions: 310,
    ctr: 6.17,
    startDate: '2025-04-10',
    endDate: '2025-04-30',
  },
  {
    id: 'cmp-7',
    name: 'Influencer Collab — May',
    status: 'draft',
    channel: 'social',
    budget: 25000,
    spent: 0,
    impressions: 0,
    clicks: 0,
    conversions: 0,
    ctr: 0,
    startDate: '2025-05-01',
    endDate: '2025-05-31',
  },
  {
    id: 'cmp-8',
    name: 'Loyalty Reactivation',
    status: 'completed',
    channel: 'email',
    budget: 6000,
    spent: 5980,
    impressions: 142000,
    clicks: 9300,
    conversions: 1120,
    ctr: 6.55,
    startDate: '2025-03-01',
    endDate: '2025-03-31',
  },
]

export const linkedEntities: Record<string, LinkedEntity[]> = {
  'cmp-7': [
    {
      id: 'le-1',
      name: 'Influencer Agency Pro',
      slotsAvailable: 45,
      commissionOffered: 35,
    },
    {
      id: 'le-2',
      name: 'Creative Collective Ltd',
      slotsAvailable: 32,
      commissionOffered: 28,
    },
    {
      id: 'le-3',
      name: 'Digital Stars Network',
      slotsAvailable: 58,
      commissionOffered: 42,
    },
    {
      id: 'le-4',
      name: 'Content Kings Media',
      slotsAvailable: 28,
      commissionOffered: 22,
    },
    {
      id: 'le-5',
      name: 'Social Connect Agency',
      slotsAvailable: 41,
      commissionOffered: 38,
    },
  ],
  'cmp-6': [
    {
      id: 'le-6',
      name: 'Email Marketing Experts',
      slotsAvailable: 12,
      commissionOffered: 18,
    },
    {
      id: 'le-7',
      name: 'Outreach Specialists',
      slotsAvailable: 18,
      commissionOffered: 25,
    },
    {
      id: 'le-8',
      name: 'Campaign Masters',
      slotsAvailable: 25,
      commissionOffered: 32,
    },
  ],
}

export const availableEntities: LinkedEntity[] = [
  {
    id: 'le-1',
    name: 'Influencer Agency Pro',
    slotsAvailable: 45,
    commissionOffered: 35,
  },
  {
    id: 'le-2',
    name: 'Creative Collective Ltd',
    slotsAvailable: 32,
    commissionOffered: 28,
  },
  {
    id: 'le-3',
    name: 'Digital Stars Network',
    slotsAvailable: 58,
    commissionOffered: 42,
  },
  {
    id: 'le-4',
    name: 'Content Kings Media',
    slotsAvailable: 28,
    commissionOffered: 22,
  },
  {
    id: 'le-5',
    name: 'Social Connect Agency',
    slotsAvailable: 41,
    commissionOffered: 38,
  },
  {
    id: 'le-6',
    name: 'Email Marketing Experts',
    slotsAvailable: 12,
    commissionOffered: 18,
  },
  {
    id: 'le-7',
    name: 'Outreach Specialists',
    slotsAvailable: 18,
    commissionOffered: 25,
  },
  {
    id: 'le-8',
    name: 'Campaign Masters',
    slotsAvailable: 25,
    commissionOffered: 32,
  },
  {
    id: 'le-9',
    name: 'Brand Boost Network',
    slotsAvailable: 35,
    commissionOffered: 30,
  },
  {
    id: 'le-10',
    name: 'Performance Partners Inc',
    slotsAvailable: 50,
    commissionOffered: 40,
  },
  {
    id: 'le-11',
    name: 'Social Media Syndicate',
    slotsAvailable: 22,
    commissionOffered: 26,
  },
  {
    id: 'le-12',
    name: 'Viral Campaigns Ltd',
    slotsAvailable: 40,
    commissionOffered: 33,
  },
]

export const performanceBars: PerformanceBar[] = [
  { label: 'Email',   value: 68, max: 100, accent: 'cyan'   },
  { label: 'Social',  value: 84, max: 100, accent: 'purple' },
  { label: 'Search',  value: 91, max: 100, accent: 'green'  },
  { label: 'Display', value: 42, max: 100, accent: 'blue'   },
  { label: 'Video',   value: 57, max: 100, accent: 'orange' },
]

export const securityQuestions: SecurityQuestion[] = [
  'What was the name of your first pet?',
  "What is your mother's maiden name?",
  'What city were you born in?',
  'What was the name of your first school?',
  "What is your oldest sibling's middle name?",
  'What street did you grow up on?',
  'What was the make of your first car?',
  'What is your favourite movie?',
]

export const currentUser: UserDetails = {
  code: 'BRD-A7K2P',
  type: 'brand',
  name: 'Acme Corp',
  mobile: '+91 9876543210',
}

export const deals: RawDeal[] = [
  {
    id: 'd-1',
    productName: 'boAt Rockerz 450 Bluetooth On-Ear Headphones',
    productImageUrl: 'https://m.media-amazon.com/images/I/61eD3sEoJKL._SX679_.jpg',
    platform: 'PLATFORM_AMAZON',
    dealType: 'CAMPAIGN_TYPE_REVIEW',
    originalPricePaise: 299900,
    offeredPricePaise: 149900,
    status: 'explore',
  },
  {
    id: 'd-2',
    productName: 'Lakme Absolute Matte Revolution Lip Color',
    productImageUrl: 'https://www.nykaa.com/media/catalog/product/3/5/35014_1.jpg',
    platform: 'PLATFORM_NYKAA',
    dealType: 'CAMPAIGN_TYPE_RATING',
    originalPricePaise: 79900,
    offeredPricePaise: 63900,
    status: 'explore',
  },
  {
    id: 'd-3',
    productName: 'Nike Air Max 270 React Running Shoes',
    productImageUrl: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=400&q=80',
    platform: 'PLATFORM_MYNTRA',
    dealType: 'CAMPAIGN_TYPE_DISCOUNT',
    originalPricePaise: 1299900,
    offeredPricePaise: 779900,
    status: 'explore',
  },
  {
    id: 'd-4',
    productName: 'Samsung 65" QLED 4K Smart TV',
    productImageUrl: 'https://images.unsplash.com/photo-1593305841991-05c297ba4575?w=400&q=80',
    platform: 'PLATFORM_FLIPKART',
    dealType: 'CAMPAIGN_TYPE_ORDER',
    originalPricePaise: 12999900,
    offeredPricePaise: 9999900,
    status: 'explore',
  },
  {
    id: 'd-5',
    productName: 'The Ordinary Niacinamide 10% + Zinc 1% Serum',
    productImageUrl: 'https://www.nykaa.com/media/catalog/product/8/4/8432592_1.jpg',
    platform: 'PLATFORM_NYKAA',
    dealType: 'CAMPAIGN_TYPE_REVIEW',
    originalPricePaise: 69900,
    offeredPricePaise: 55900,
    status: 'explore',
  },
  {
    id: 'd-6',
    productName: 'Instant Pot Duo 7-in-1 Electric Pressure Cooker',
    productImageUrl: 'https://images.unsplash.com/photo-1585515320310-259814833e62?w=400&q=80',
    platform: 'PLATFORM_AMAZON',
    dealType: 'CAMPAIGN_TYPE_RATING',
    originalPricePaise: 899900,
    offeredPricePaise: 674900,
    status: 'explore',
  },
  {
    id: 'd-7',
    productName: 'boAt Rockerz 450 Bluetooth On-Ear Headphones',
    productImageUrl: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&q=80',
    platform: 'PLATFORM_AMAZON',
    dealType: 'CAMPAIGN_TYPE_REVIEW',
    originalPricePaise: 199900,
    offeredPricePaise: 149900,
    status: 'in_progress',
    claimStep: 2,
    claimStatus: 'pending',
    claimOrder: {
      orderId: 'AMZ-2025-0042',
      amount: '₹1,499',
      productName: 'boAt Rockerz 450 Bluetooth On-Ear Headphones',
      soldBy: 'boAt Official',
      orderDate: '28 Apr 2025',
      accountName: 'payam@coddicted.com',
      screenshotUrl: 'https://images.unsplash.com/photo-1563986768494-4dee2763ff3f?w=600&q=80',
    },
  },
  {
    id: 'd-8',
    productName: 'Mamaearth Vitamin C Face Wash with Turmeric',
    productImageUrl: 'https://images.unsplash.com/photo-1556228578-0d85b1a4d571?w=400&q=80',
    platform: 'PLATFORM_NYKAA',
    dealType: 'CAMPAIGN_TYPE_RATING',
    originalPricePaise: 29900,
    offeredPricePaise: 23900,
    status: 'in_progress',
    claimStep: 3,
    claimStatus: 'pending',
    claimOrder: {
      orderId: 'NYK-2025-1187',
      amount: '₹239',
      productName: 'Mamaearth Vitamin C Face Wash with Turmeric',
      soldBy: 'Mamaearth',
      orderDate: '25 Apr 2025',
      accountName: 'payam@coddicted.com',
    },
  },
  {
    id: 'd-9',
    productName: 'Levi\'s Men\'s 511 Slim Fit Jeans',
    productImageUrl: 'https://images.unsplash.com/photo-1542272604-787c3835535d?w=400&q=80',
    platform: 'PLATFORM_MYNTRA',
    dealType: 'CAMPAIGN_TYPE_ORDER',
    originalPricePaise: 399900,
    offeredPricePaise: 279900,
    status: 'in_progress',
    claimStep: 3,
    claimStatus: 'approved',
    claimOrder: {
      orderId: 'MYN-2025-7731',
      amount: '₹2,799',
      productName: 'Levi\'s Men\'s 511 Slim Fit Jeans',
      soldBy: 'Levi\'s India',
      orderDate: '20 Apr 2025',
      accountName: 'payam@coddicted.com',
    },
    claimPayout: {
      amount: '₹1,200',
      disbursedOn: '2025-05-03',
    },
  },
  {
    id: 'd-10',
    productName: 'Philips AirFryer HD9200/10 — 4.1L',
    productImageUrl: 'https://images.unsplash.com/photo-1648723898083-b5bffda76d7a?w=400&q=80',
    platform: 'PLATFORM_FLIPKART',
    dealType: 'CAMPAIGN_TYPE_DISCOUNT',
    originalPricePaise: 799900,
    offeredPricePaise: 499900,
    status: 'in_progress',
    claimStep: 4,
    claimStatus: 'approved',
    claimOrder: {
      orderId: 'FLP-2025-3309',
      amount: '₹4,999',
      productName: 'Philips AirFryer HD9200/10',
      soldBy: 'Philips India',
      orderDate: '15 Apr 2025',
      accountName: 'payam@coddicted.com',
    },
    claimPayout: {
      amount: '₹3,000',
      disbursedOn: '2025-05-01',
    },
  },
]

export const dealTypes: DealTypeOption[] = [
  { value: 'CAMPAIGN_TYPE_RATING',   label: 'Rating',   activeClass: 'bg-neon-yellow/10 text-neon-yellow border-neon-yellow/30'  },
  { value: 'CAMPAIGN_TYPE_REVIEW',   label: 'Review',   activeClass: 'bg-neon-cyan/10   text-neon-cyan   border-neon-cyan/30'    },
  { value: 'CAMPAIGN_TYPE_DISCOUNT', label: 'Discount', activeClass: 'bg-neon-red/10    text-neon-red    border-neon-red/30'     },
  { value: 'CAMPAIGN_TYPE_ORDER',    label: 'Order',    activeClass: 'bg-neon-green/10  text-neon-green  border-neon-green/30'   },
]

export const platforms: PlatformOption[] = [
  { value: 'PLATFORM_AMAZON',   label: 'Amazon'   },
  { value: 'PLATFORM_FLIPKART', label: 'Flipkart' },
  { value: 'PLATFORM_NYKAA',    label: 'Nykaa'    },
  { value: 'PLATFORM_MYNTRA',   label: 'Myntra'   },
]

export const connections: Connection[] = [
  { id: 'c-1',  name: 'Nova Retail',       type: 'brand',  category: 'Fashion & Apparel', status: 'connected', since: 'Jan 2025', avatar: 'N', avatarColor: '#57c7ff' },
  { id: 'c-2',  name: 'Apex Media',        type: 'agency', category: 'Performance',       status: 'connected', since: 'Mar 2025', avatar: 'A', avatarColor: '#bd93f9' },
  { id: 'c-3',  name: 'GreenLeaf Co.',     type: 'brand',  category: 'Health & Wellness', status: 'connected', since: 'Apr 2025', avatar: 'G', avatarColor: '#50fa7b' },
  { id: 'c-4',  name: 'Luminary Agency',   type: 'agency', category: 'Influencer',        status: 'pending',                     avatar: 'L', avatarColor: '#ffb86c' },
  { id: 'c-5',  name: 'Orbit Electronics', type: 'brand',  category: 'Consumer Tech',     status: 'pending',                     avatar: 'O', avatarColor: '#ff79c6' },
  { id: 'c-6',  name: 'Stride Sports',     type: 'brand',  category: 'Sports & Fitness',  status: 'invited',                     avatar: 'S', avatarColor: '#ff5555' },
  { id: 'c-7',  name: 'Beacon Digital',    type: 'agency', category: 'Content',           status: 'invited',                     avatar: 'B', avatarColor: '#8be9fd' },
]

