...
import { searchCampaigns, shareCampaign } from '../../../api/campaignApi'
import { emptyFilters } from '../campaign/filters/CampaignFilterTypes'
...
export function ShareCampaignView({ connection, onBack }: Props) {
  ...
  const loadCampaigns = useCallback(() => {
    setLoading(true)
    searchCampaigns(emptyFilters(), 0, 100)
  ...
  async function handleConfirmShare() {
    ...
    setSharing(true)
    try {
      await shareCampaign(pendingCampaign.id, connection.toUserId)
    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-5">
      ...
      <div>
        <h1 className="text-xl font-bold text-ink-light-primary dark:text-ink-dark-primary">
          Share a Campaign with {connection.name}
        </h1>
      </div>
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-surface-light-hover dark:hover:bg-surface-dark-hover transition-colors">
                      <td className="px-5 py-2.5 w-14">
                        <ProductThumbnail src={c.productImageUrl} alt={c.title} />
                      </td>
